#!/usr/bin/env python3
"""Stage Randomizer for all v1.0.1 release zip(s).

Modes: --mode exe | scripts | both (default both).
Contract: .ai/docs/发行包契约.md §2 / §12
"""
...
def stage_data(stage: Path, *, pack_mode: str = "exe") -> None:
    data = stage / "data"
    if data.exists():
        shutil.rmtree(data)
    ...
    if pack_mode == "scripts":
        _stage_poc_sources_no_exe(data)
    else:
        _stage_poc_prebuilt(data)

    _sanitize_shipped_json_paths(data)


def _stage_poc_prebuilt(data: Path) -> None:
    """Full exe pack: ship prebuilt helper tools (includes .exe)."""
...
_POC_CS_IGNORE = shutil.ignore_patterns(
    "bin",
    "obj",
    ".vs",
    "*.user",
...
def _write_release_tool_csproj(dst: Path, *, project_name: str) -> None:
    """Portable csproj: no developer-machine absolute paths."""
    excludes = ""
    if project_name == "NpcSoulPatch":
        excludes = """
  <PropertyGroup>
    <DefaultItemExcludes>$(DefaultItemExcludes);_PeekNpc\\**;_PeekBnd\\**;_Dump.cs</DefaultItemExcludes>
  </PropertyGroup>
  <ItemGroup>
    <Compile Remove="_Dump.cs" />
    <None Include="_Dump.cs" />
    <Compile Remove="_PeekBnd.cs" />
    <None Include="_PeekBnd.cs" />
    <Compile Remove="_PeekBnd\\**" />
    <None Include="_PeekBnd\\**" />
    <Compile Remove="_PeekNpc\\**" />
    <None Include="_PeekNpc\\**" />
  </ItemGroup>"""
    text = f"""<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <OutputType>Exe</OutputType>
    <TargetFramework>net8.0</TargetFramework>
    <ImplicitUsings>enable</ImplicitUsings>
    <Nullable>enable</Nullable>
  </PropertyGroup>{excludes}
  <ItemGroup>
    <ProjectReference Include="..\\_sfnext\\SoulsFormats\\SoulsFormats.csproj" />
  </ItemGroup>
  <ItemGroup>
    <None Include="..\\_native\\oo2core_6_win64.dll"
          Link="oo2core_6_win64.dll"
          CopyToOutputDirectory="PreserveNewest"
          Condition="Exists('..\\_native\\oo2core_6_win64.dll')" />
  </ItemGroup>
</Project>
"""
...
def _stage_poc_sources_no_exe(data: Path) -> None:
    """Scripts pack: C# sources only — hard zero .exe in tree."""
    poc_dst = data / "cnv_enemy_poc"
    poc_dst.mkdir(parents=True, exist_ok=True)

    for proj in ("MsbEnemyPoc", "NpcSoulPatch"):
        src = POC_DIR / proj
        dst = poc_dst / proj
        if not (src / f"{proj}.csproj").is_file():
            raise SystemExit(f"missing project {src}")
        if dst.exists():
            shutil.rmtree(dst)
        shutil.copytree(src, dst, ignore=_POC_CS_IGNORE)
        _write_release_tool_csproj(dst / f"{proj}.csproj", project_name=proj)

    sf_src = POC_DIR / "_sfnext"
    sf_dst = poc_dst / "_sfnext"
    if not (sf_src / "SoulsFormats" / "SoulsFormats.csproj").is_file():
        raise SystemExit(f"missing SoulsFormats at {sf_src}")
    if sf_dst.exists():
        shutil.rmtree(sf_dst)
    shutil.copytree(
        sf_src,
        sf_dst,
        ignore=shutil.ignore_patterns(
    ...
    native = poc_dst / "_native"
    native.mkdir(parents=True, exist_ok=True)
    oo2_candidates = [
        POC_DIR / "MsbEnemyPoc" / "bin" / "Release" / "net8.0" / "oo2core_6_win64.dll",
        Path(r"V:\games\Elden Ring\Game\tools\DSMSPortable\app\oo2core_6_win64.dll"),
    ]
    oo2_ok = False
    for cand in oo2_candidates:
        if cand.is_file():
            shutil.copy2(cand, native / "oo2core_6_win64.dll")
            oo2_ok = True
            break
    if not oo2_ok:
        print("[warn] oo2core_6_win64.dll not found for scripts pack _native/", flush=True)

    # Safety: strip any .exe that slipped through copytree
    for exe in poc_dst.rglob("*.exe"):
...
SCRIPTS_PACK_DIR = Path(__file__).resolve().parent / "scripts_pack"
ZIP_EXE_NAME = f"{PRODUCT}-{VERSION}.zip"
ZIP_SCRIPTS_NAME = f"{PRODUCT}-{VERSION}-scripts.zip"


def write_docs(stage: Path, prep_ver: int, *, mode: str) -> None:
    """mode: exe | scripts"""
    pack_kind = "exe-onedir" if mode == "exe" else "scripts-python"
    ...
    (stage / "CHANGELOG.txt").write_text(
        ...
        "- Dual-track release: full exe zip + scripts zip (Nexus-friendly)\n"
        "- Current support: The Convergence 3.0 + ME3 only\n",
        encoding="utf-8",
    )
    if mode == "scripts":
        _write_scripts_docs(stage)
    else:
        _write_exe_docs(stage)


def _write_exe_docs(stage: Path) -> None:
    cn = stage / "README_安装_中文.txt"
    cn.write_text(
        f"""Randomizer for all {VERSION}（完整版 · 带界面程序）
================================

本包用于：法魂 Convergence 3.0 + ME3。
需要：Windows 10/11、.NET 8 运行时、已能正常进法魂的游戏目录。
（本包界面程序不强制安装 Python；改地图的小工具仍要 .NET 8。）
...
渠道说明
--------
若你在 N 网只看到「脚本版」包：那是无本界面程序的版本（给审核更干净）。
完整带界面程序的包一般在码云发布页。
...
""",
    ...
    en.write_text(
        f"""Randomizer for all {VERSION} (full pack with GUI exe)
================================

For: The Convergence 3.0 + ME3.
Requires: Windows 10/11, .NET 8 Runtime, a working Convergence install.
(This GUI exe does not require Python; map helper tools still need .NET 8.)
...
6. Fully quit the game, then launch Convergence as usual (often Start_Convergence_ME3.bat).

Channel note
------------
Nexus may host the scripts pack (no PyInstaller GUI). The full GUI zip is usually on Gitee.
...
def _write_scripts_docs(stage: Path) -> None:
    cn = stage / "README_安装_中文.txt"
    cn.write_text(
        f"""Randomizer for all {VERSION}（脚本版 · 无界面 exe）
================================

本包用于：法魂 Convergence 3.0 + ME3。
本包故意不含任何 .exe（含界面大程序与敌人小工具预编译），方便 N 网审核。
功能与完整版相同：用本机 Python 跑同一套程序；敌人小工具需要你本机用 .NET 8 SDK 编译一次。

需要
----
1. Windows 10/11
2. 已能进游戏的法魂 Convergence 3.0 + ME3
3. Python 3.13（官方安装，务必勾选 Add to PATH）
4. .NET 8 SDK（不是只装运行时；要用它编译写地图的小工具）

推荐步骤
--------
1. 解压本包到短路径目录。
2. 双击 0_安装运行环境.bat（英文用户用 0_InstallRuntime.bat）
   - 尽量自动装 Python / .NET 8 SDK，并编译两个小工具
   - 没有 winget 则打开官网，请你手动装完后重新运行本 bat
3. 双击 1_打开界面.bat（英文用户用 1_OpenGUI.bat）
   - 黑窗会提示「请稍候」，**等界面窗口出现后再自动关闭**（避免误以为没打开）
   - 在界面「安装mod到游戏」页选 Game 目录并安装
   - 再设种子/选项 →「开始生成」
4. （可选）也可改 data\\cnv_randomizer\\config.json —— 见 CONFIG字段说明_中文.txt
5. 完全退出游戏 → 用法魂原来的方式进游戏验证

注意
----
- 中文 bat 已按 Windows 中文系统常用编码（GBK）写出；若用错误编码另存会再次乱码，请不要用记事本另存为 UTF-8。
- 发行 zip 内保证零 .exe；编译出的小工具只会出现在你解压后的本机目录。
- 不要把完整 Python 或 .NET 安装包塞进本 zip；请用官方安装。
- 完整带 exe 界面的包见码云发布页（本脚本版给 N 网）。
- 装模组/生成都在界面里；包根只提供「装运行时」和「开界面」。

手动命令（高级）
----------------
  cd data\\cnv_randomizer
  python player_run.py doctor
  python player_run.py install-mod --game-dir "D:\\Games\\Elden Ring\\Game"
  python player_run.py gui
  python player_run.py generate
""",
        encoding="utf-8",
    )
    en = stage / "README_Install_EN.txt"
    en.write_text(
        f"""Randomizer for all {VERSION} (scripts pack · zero .exe in the zip)
================================

For Convergence 3.0 + ME3. This zip contains **no .exe at all** (Nexus-friendly):
no PyInstaller GUI and no prebuilt map helper tools. Same features via local Python;
helpers are built once on your PC with the .NET 8 SDK.

Requires: Windows 10/11, Python 3.13 (Add to PATH), .NET 8 SDK.

Steps
-----
1. Extract to a short path.
2. Run 0_InstallRuntime.bat (Chinese users: 0_安装运行环境.bat)
   — installs Python / .NET 8 SDK when possible, then builds the two helper tools.
3. Run 1_OpenGUI.bat (Chinese users: 1_打开界面.bat)
   — black window stays with "please wait" until the GUI appears, then closes.
   → Install mod tab → Generate in the GUI.
4. Optional: edit data\\cnv_randomizer\\config.json
5. Fully quit the game, launch Convergence as usual.

Encoding note
-------------
Chinese .bat files are saved as GBK (Windows Chinese code page), not UTF-8.
English .bat files are ASCII. Do not re-save Chinese bats as UTF-8 or text will garble in cmd.

Install-mod / generate-only bats are not shipped — use the GUI.

Full GUI exe zip is published on Gitee. This scripts pack is for Nexus.
""",
        encoding="utf-8",
    )
    (stage / "CONFIG字段说明_中文.txt").write_text(
        """config.json 常用字段（白话）
============================
文件位置：data\\cnv_randomizer\\config.json
可用记事本改；改完保存，再打开界面点「开始生成」。

seed
  随机种子（整数）。相同种子 + 相同选项 ≈ 相同结果。

enemy_gui.page_items_enabled
  true/false：是否随机地上物品。

enemy_gui.page_enemies_enabled
  true/false：是否随机敌人。

enemy_gui.category_weights
  怪物类别互相替换的比例（界面「怪物」页那张表）。

enemy_gui.difficulty.user_mult
  难度倍率：约 0.25 新手 · 0.5 正常 · 0.75 困难 · ≥1 更难。

enemy_gui.auto_randomize_seed
  true 时界面每次生成会换新种子（CLI generate 以文件里 seed 为准）。

randomize_categories / randomize_goods_subcats
  哪些装备 / 道具大类、细类参与物品随机。
...
不要改
------
csv_dir、exclude_item_ids 大列表、skip_lot_* 等高级项，除非你清楚在做什么。
发行包请保持 maintainer_gui 为 false。
""",
        encoding="utf-8",
    )


def copy_scripts_pack_bats(stage: Path) -> None:
    """Write ASCII bat launchers into scripts zip root (cmd-safe encoding)."""
    import importlib.util

    emit_py = SCRIPTS_PACK_DIR / "_emit_bats.py"
    if not emit_py.is_file():
        raise SystemExit(f"missing {emit_py}")
    spec = importlib.util.spec_from_file_location("scripts_pack_emit_bats", emit_py)
    if spec is None or spec.loader is None:
        raise SystemExit(f"cannot load {emit_py}")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    written = mod.emit(stage)
    if len(written) < 4:
        raise SystemExit(f"scripts pack bats missing after emit: {len(written)}")
    # Spot-check encoding contract
    by_name = {p.name: p for p in written}
    zh = by_name["0_安装运行环境.bat"].read_bytes()
    zh.decode("gbk")
    try:
        zh.decode("utf-8")
        ...
        # If somehow valid UTF-8, still require it differs from utf-8 encode of same text
        # Real CN GBK body always has bytes that are invalid UTF-8 for CJK phrases we use.
        raise SystemExit("Chinese bat unexpectedly decodes as UTF-8; expected GBK")
    by_name["0_InstallRuntime.bat"].read_bytes().decode("ascii")


def assert_scripts_no_gui_exe(stage: Path) -> None:
    """Scripts pack: no PyInstaller GUI and hard zero .exe anywhere (contract §2.2)."""
    bad: list[str] = []
    if (stage / f"{APP_NAME}.exe").exists():
        bad.append(f"{APP_NAME}.exe")
    if (stage / "_internal").exists():
        bad.append("_internal/")
    for exe in stage.rglob("*.exe"):
        bad.append(exe.relative_to(stage).as_posix())
    if bad:
        raise SystemExit(
            "scripts pack must not contain any .exe (contract §2.2):\n  "
            + "\n  ".join(bad[:40])
        )
...
def make_zip(stage: Path, *, zip_name: str, arc_root: str) -> Path:
        ...
        for path in stage.rglob("*"):
            if path.is_file():
                ...
                if path.name == "game_path.json":
                    continue
                arc = f"{arc_root}/{path.relative_to(stage).as_posix()}"
...
def cleanup_dist_keep_zips(keep_names: list[str]) -> None:
    """Contract: dist/ must only contain the finished release zip(s)."""
    import time
    ...
    if left != sorted(keep_names):
        raise SystemExit(f"dist/ cleanup failed, still has: {left} expected {keep_names}")


def _prep_ver_for_docs() -> int:
    sys.path.insert(0, str(CNV_DIR))
    from enemy_slot_prep import PREP_VERSION, read_slot_prep_meta_record

    meta = read_slot_prep_meta_record()
    return int((meta or {}).get("version", PREP_VERSION))
...
def build_one_mode(
    ...
    *,
    ...
    shared_prep_done: bool,
) -> Path:
    """Build one zip; returns zip path. Caller runs prep/dotnet/dll once."""
    if mode not in ("exe", "scripts"):
        raise SystemExit(f"bad mode {mode}")

    stage = DIST_ROOT / f"_stage_{PRODUCT}-{VERSION}-{mode}"
    if stage.exists():
        shutil.rmtree(stage)
    ...
    stage_data(stage, pack_mode=mode)
    assert_shipped_prep_ready(stage)
    prep_ver = _prep_ver_for_docs()
    write_docs(stage, prep_ver, mode=mode)

    if mode == "scripts":
        copy_scripts_pack_bats(stage)
        assert_scripts_no_gui_exe(stage)
    ...
    if mode == "exe":
        zip_path = make_zip(
            stage,
            zip_name=ZIP_EXE_NAME,
            arc_root=f"{PRODUCT}-{VERSION}",
        )
    else:
        zip_path = make_zip(
            stage,
            zip_name=ZIP_SCRIPTS_NAME,
            arc_root=f"{PRODUCT}-{VERSION}-scripts",
        )

    shutil.rmtree(stage, ignore_errors=True)
    print(
        f"[ok] zip: {zip_path} ({zip_path.stat().st_size // (1024 * 1024)} MB) mode={mode}",
        flush=True,
    )
    return zip_path
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Stage Randomizer for all release zip(s). Contract: 发行包契约.md §12"
    )
    ap.add_argument(
        "--mode",
        choices=("exe", "scripts", "both"),
        default="both",
        help="exe=完整版 · scripts=脚本版 · both=两个都打（默认）",
    )
    ...
    # scripts-only: skip pyinstaller entirely
    skip_pyi = bool(args.skip_pyinstaller) or (modes == ["scripts"])
    ...
    for mode in modes:
        zips.append(
            build_one_mode(
                mode,
                skip_pyinstaller=skip_pyi if mode == "exe" else True,
                shared_prep_done=True,
            )
        )

    keep = [z.name for z in zips]
    cleanup_dist_keep_zips(keep)
    print(f"[ok] dist/ contains only: {', '.join(keep)}")
    return 0
