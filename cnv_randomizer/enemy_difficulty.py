...
def normalize_difficulty_settings(enemy_gui: dict[str, Any] | None) -> dict[str, Any]:
    ...
    try:
        lo = float(t093.get("user_mult_min", 0.01) if t093_enabled(cfg) else out.get("user_mult_min", 0.25))
        hi = float(t093.get("user_mult_max", 999.0) if t093_enabled(cfg) else out.get("user_mult_max", 2.0))
...
def resolve_t093_target_effective_hp(
    ...
    *,
    ...
    """白名单 → 夜 → 周目层lift → 硬帽 → 滑条lift → 地图层lift（最后）。

    2026-09-09：地图放滑条后，避免 0.25 下高血 Boss 难图反比早图薄。
    """
    ...
    # 地图层最后：同皮高档最终血不低于低档（滑条先压高血）
    if use_t094:
        map_eff = map_layer_lift_mult(h, map_nom, difficulty_cfg=difficulty_cfg)
        h *= map_eff
    else:
        h *= map_nom
...
def rune_category_uses_hp_balance(
    ...
    excluded = {
        str(x)
        for x in (
            difficulty_cfg.get("rune_hp_scale_exclude_categories") or ["major_boss", "night"]
        )
    }
...
def compute_t093_stat_lift_mult(
    ...
    *,
    ...
    """韧/视/嗅等：周目层 → 硬帽 → 滑条 → 地图层（最后；与 HP 叠序对齐）。"""
    from enemy_whitelist_hp import resolve_donor_baseline_effective_hp
    from enemy_world_progression import (
        asymmetric_layer_lift_mult,
        compute_slider_lift_mult,
    ...
    if use_t094:
        m_eff = map_layer_lift_mult(h, map_nom, difficulty_cfg=difficulty_cfg)
        out *= m_eff
...
def compute_t093_attack_lift_mult(
    ...
    *,
    ...
    攻击路径无全局滑条乘子；地图层即为最终 mod 攻倍率（与 HP「地图放最后」不冲突）。
    """
    from enemy_whitelist_hp import resolve_donor_baseline_effective_hp
    from enemy_world_progression import (
        map_layer_lift_mult,
