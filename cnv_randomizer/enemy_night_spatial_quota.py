...
def load_major_boss_spatial_quota_config(
    ...
    """T-069：6 池 major_boss 空间配额（现网默认 50×50×高10 · 图内格）。"""
...
def major_boss_spatial_cell_key(
    ...
    *,
    ...
    """T-069（2026-09-23）：一律图内切格 ``(map_id, ix, iy, iz)``，不跨图 ow、不整图塌缩。"""
...
def is_major_boss_spatial_quota_exempt(
    ...
    """洞窟终点 ``_9000`` 或雾门主线战点：占坑但不被降池。"""

    cfg = categories_cfg if isinstance(categories_cfg, dict) else {}

    from boss_npc_detect import is_indoor_dungeon_boss_arena_slot

    from enemy_category_rules import is_scripted_dungeon_main_boss_slot



    return bool(

        is_indoor_dungeon_boss_arena_slot(slot, cfg)

        or is_scripted_dungeon_main_boss_slot(slot, cfg)
